...
async function main(contract_name: string) {
    ...
    const testSierra = json.parse(fs.readFileSync(`./target/dev/workshop_${contract_name}.contract_class.json`).toString("ascii"));
    const testCasm = json.parse(fs.readFileSync(`./target//dev/workshop_${contract_name}.compiled_contract_class.json`).toString("ascii"));
